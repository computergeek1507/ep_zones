...
class RadarPainter extends CustomPainter {
  ...
  void _drawGrid(Canvas canvas) {
    ...
    for (double x = startX; x <= device.maxX; x += step) {
      ...
      canvas.drawLine(p1, p2, x == 0 ? (grid..color = const Color(0xFF35506B)) : grid);
      grid.color = const Color(0xFF1E2A38);
    }
    final startY = (device.minY / step).ceil() * step;
    for (double y = startY; y <= device.maxY; y += step) {
      canvas.drawLine(
          t.toCanvas(device.minX, y), t.toCanvas(device.maxX, y), grid);
      _label(canvas, '${(y / 1000).toStringAsFixed(0)}m',
          t.toCanvas(device.minX, y) + const Offset(2, 2),
          const Color(0xFF54708A), 10);
    }
  }
  ...
  void _drawZone(Canvas canvas, EpZone z, bool selected) {
    ...
    final color = z.occupied ? const Color(0xFF00E676) : const Color(0xFF35C2F0);
    canvas.drawRect(
        rect, Paint()..color = color.withValues(alpha: z.occupied ? 0.22 : 0.10));
    canvas.drawRect(
        rect,
        Paint()
    ...
    if (selected) {
      for (final c in _corners(z)) {
        ...
        canvas.drawCircle(
            cp, RadarPainter.handleRadius, Paint()..color = Colors.white);
        canvas.drawCircle(
            cp,
    ...
    _zoneLabel(canvas, 'Z${z.index}${z.count > 0 ? "  •${z.count}" : ""}', rect,
        color, selected);
  }

  void _zoneLabel(
      Canvas canvas, String text, Rect rect, Color color, bool selected) {
